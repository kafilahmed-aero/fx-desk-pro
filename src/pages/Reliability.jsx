...
import { Award, AlertCircle, CheckCircle2, ShieldAlert, TrendingUp, Clock, HelpCircle } from "lucide-react";
...
function Reliability() {
  ...
  useEffect(() => {
    ...
    fetchWithCredentials("/channel-performance")
      .then(async (res) => {
        if (!res.ok) {
        ...
        if (isMounted) {
          ...
          // 1. isReliabilityEligible DESC
          // 2. winRate DESC
          // 3. completedSignals DESC
          const sorted = [...json].sort((a, b) => {
            const eligibleA = a.isReliabilityEligible ? 1 : 0;
            const eligibleB = b.isReliabilityEligible ? 1 : 0;
    ...
    return (
      <div className="rounded-2xl border border-rose-200 bg-rose-50 p-5 dark:border-rose-500/20 dark:bg-rose-500/10">
        <p className="flex items-center gap-2 text-sm font-bold text-rose-600 dark:text-rose-400">
          <AlertCircle size={18} />
          Error Loading Reliability Page
        </p>
    ...
    <div className="animate-dashboard-in">
      ...
      <div className="mt-6 flex flex-col gap-4 rounded-2xl border border-blue-100 bg-blue-50/50 p-4 dark:border-blue-500/10 dark:bg-blue-950/20 sm:flex-row sm:items-start">
        ...
        <div className="text-xs leading-5 text-slate-600 dark:text-slate-300">
          <span className="font-bold">Statistical Eligibility Rules:</span> A signal channel must reach at least <strong>20 completed signals</strong> before it is marked as <strong>Reliability Eligible</strong>. Channels that are not yet eligible are sorted to the bottom. Win Rate color boundaries are defined as Green (≥ 60%), Yellow (45% - 59%), and Red (&lt; 45%).
        </div>
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full border-collapse text-left text-sm">
            ...
            <tbody className="divide-y divide-slate-200/65 dark:divide-white/5">
              {data.length === 0 ? (
                <tr>
                  <td colSpan="11" className="px-5 py-8 text-center text-xs font-medium text-slate-400 dark:text-slate-500">
                    No channel reliability records available.
                  </td>
                </tr>
              ) : (
                data.map((item) => (
                    ...
                    <td className="px-5 py-4.5">
                      {item.isReliabilityEligible ? (
                        ...
                        <span className="inline-flex items-center gap-1.5 rounded-lg bg-slate-500/10 px-2 py-1 text-xs font-bold text-slate-500">
                          <ShieldAlert size={13} />
                          Pending ({(item.completedSignals || 0)}/20)
                        </span>
                      )}
                    </td>

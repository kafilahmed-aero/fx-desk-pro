...
export async function getSystemHealthController(req, res) {
  try {
    ...
    return res.status(200).json({
      ...
      telegram: {
        status: telegramMetrics.connected ? "CONNECTED" : "DISCONNECTED",
        listenerRunning: telegramMetrics.listenerRunning,
        connected: telegramMetrics.connected,
        polling: telegramMetrics.pollingInProgress ? "ACTIVE" : "INACTIVE",
        reconnectAttempts: telegramMetrics.reconnectAttempts,
        pollCycles: telegramMetrics.pollCycles,
        messagesFetched: telegramMetrics.messagesFetched,
        messagesStored: telegramMetrics.messagesStored,
        queuedMessagesCount: telegramMetrics.queuedMessagesCount,
        lastPollTime: telegramMetrics.lastPollTime,
        lastReconnectTime: telegramMetrics.lastReconnectTime,
        lastError: telegramMetrics.lastError,
...
export async function getTelegramHealthController(req, res) {
  try {
    ...
    return res.status(200).json({
      ...
      pollCycles: metrics.pollCycles,
      messagesFetched: metrics.messagesFetched,
      messagesStored: metrics.messagesStored,
      queuedMessagesCount: metrics.queuedMessagesCount,
      lastPollTime: metrics.lastPollTime,
      lastReconnectTime: metrics.lastReconnectTime,
